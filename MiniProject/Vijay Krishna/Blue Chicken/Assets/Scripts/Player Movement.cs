...
public class PlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Ensure UI is initialized with current seed count
    ...
    IEnumerator AnimateFeatherDrop(GameObject feather)
{
    ...
    /// <summary>
    /// Checks if the player has seeds and consumes one if available.
    /// Used by the BulletSpawning script for firing ammo.
    /// </summary>
    /// <returns>True if a seed was consumed and the shot can fire, false otherwise.</returns>
    public bool TryConsumeSeed()
    {
        if (seedsCollected > 0)
        {
            seedsCollected--;
            OnSeedCollected?.Invoke(seedsCollected); // Notify UI to update
            return true;
        }
        return false;
    }
